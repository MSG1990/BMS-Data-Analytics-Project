...
-- creating the Chillers_run_status table
-- datetime info must be imported as a string then converted to datetime
...
load data local infile '/Users/basimyousef/Documents/BMS Data Analytics Project/BMS DATA - JAN 12/CHILLER 2 RUN_2025-12-12_02-06-57_2026-01-09_02-06-57.csv'
...
-- 2. Update only valid strings and turn empty ones into NULL
UPDATE BZ_CHILLERS_RUN_STATUS 
SET date_time = STR_TO_DATE(TRIM(`date_time`), '%Y-%m-%d %H:%i:%s')
WHERE TRIM(`date_time`) != '' AND `date_time` IS NOT NULL;
...
drop table if exists silver.sv_chillers_run_status;
...
-- deleting nulls in time:

DELETE FROM silver.sv_chillers_run_status 
WHERE date_time IS NULL 
   OR date_time = '' 
   OR date_time = ' ';

-- sensors's data time-sequencing & niose elimintation
drop table if exists `gold`.`gd_chillers_run_status`;
