...
-- verification
select * from `bz_chillers_RLA` limit 1000 offset 2000;
...
SELECT 
    STR_TO_DATE(trim(date_time), '%m/%d/%Y %H:%i:%s'),
...
drop table if exists silver.sv_chillers_RLA_finetuned;
...
drop table if exists silver.sv_chillers_RLA_finetuned_summedRLA;
...
-- to the gold schema
drop table if exists gold.gd__chillers_RLA;
...
where chiller_id = 'chiller 2' or 'chiller 4';
select * from gold.gd_chillers_RLA limit 1000 offset 2000;
select * from gold.gd_chillers_RLA limit 1000 offset 4000;
